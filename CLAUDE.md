...
admissibility, plus a closing "How Legal Capture Works" explainer. One entry
is **not** a toggle: the **Capture Timing** group's post-load wait is a number
of seconds (see step 4 below). It rides in the same options object, and so
into the sealed manifest, but is rendered and read as a number — it is not in
`LEGAL_OPTION_CHECKBOXES` and normalizes on `change` rather than on input, so
clearing the field to type a new value doesn't snap it back to 0 under the
cursor. Defaults:
...
4. Waits the operator's **post-load wait** (`postLoadWaitSeconds`, default 0, capped at 120 — `MAX_POST_LOAD_WAIT_SECONDS` in [support/legalCapture/legalCaptureOptions.js](support/legalCapture/legalCaptureOptions.js), clamped both in the popup and again on the capture side). The load event fires when the document and its subresources have arrived, which on a client-rendered page is routinely *before* the content exists — hydration, lazy images and post-load XHR all land after it, and the mutation-settle pass that follows can legitimately find the page quiet while it is merely waiting on a request that hasn't returned. This is the operator's manual override for that. It sits **here** — after the reload, before re-measurement, emulation and every snapshot — for two reasons: network recording is still running, so everything the page fetches during the wait lands in the WARC like any other traffic, and the height measured in step 5 is then taken from the settled page rather than the skeleton. Input stays suppressed throughout, so the extra time on the page is not an extra window in which to alter it. Recorded in `manifest.json`'s `captureOptions` and spelled out in `report.txt` (a package captured after a wait can contain content that did not exist at the load event, which an examiner comparing two captures of the same URL needs to know). The banner counts the seconds down — a silent minute on a page that already looks finished reads as a hang.
5. If the active resolution preset is `viewport` (User) or `fullpage` (Full Page), re-measures the tab's viewport/page height **after** the reload ([support/pageMeasure.js](support/pageMeasure.js)) rather than trusting the numbers the popup measured before the click — a fresh reload can have less lazy-loaded content on screen than whatever the popup saw a moment earlier, and using the stale numbers produces an undersized screenshot. Fixed/custom presets are used as-is (they're explicit numbers, not page-derived).
6. Emulates the (possibly re-measured) device metrics and, if enabled, takes the screenshot through the same `withZoomReset` / mutation-settle path as Page Capture. Then, in one combined `chrome.scripting.executeScript` call, optionally serializes the live, post-JS-execution `document.documentElement.outerHTML` as `page.html` and/or captures a **browser/page environment fingerprint** (screen geometry/DPI, locale, IANA timezone name, hardware thread count, document title/referrer) — independent machine-readable evidence from both the raw HTTP bodies in the WARC and the raster screenshot. Then, if enabled, captures a **full-page MHTML archive** (`page.mhtml`) via CDP `Page.captureSnapshot({format:"mhtml"})` — a browser-native, self-contained bundle of the rendered page's markup, stylesheets, scripts, images, and fonts in one file, built by Chrome itself rather than this codebase's own asset-fetching logic; opens directly in Chrome/Edge or any MHTML-capable tool. All of the above happen back-to-back against the same settled page state, so they describe the same moment. Every one of these is best-effort: a failure (e.g. a page that blocks scripting) is disclosed as absent in `report.txt`, not silently dropped. **Immediately after the screenshot and before the DOM/MHTML serialization, the extension's own injected nodes are removed** — the capture overlay and the scrollbar-hiding `<style id="__no-scroll">` — via `hideCaptureOverlay` + `restoreScrollbars`. The screenshot only needs them visually absent (`withOverlayHidden` handles that in the same CDP channel), but `page.html`/`page.mhtml` are byte-level evidence of what the site served: an examiner finding a `__sharpshooter-capture-overlay` div in them has found proof the artifact was modified after loading. Input stays suppressed across the removal, so the page never becomes editable again.
7. If network recording is on, stops recording and builds a **WARC/1.0** file from the recorded exchanges (raw request/response headers + bodies + TLS `securityDetails`) — [support/legalCapture/warcWriter.js](support/legalCapture/warcWriter.js), hand-written (no bundler/npm dependency exists in this repo, so no WARC library could be pulled in). Redirect legs are recorded as their own complete request/response pairs (the CDP `requestId` is reused across a redirect, so the recorder splits each hop into a distinct exchange rather than letting the final destination overwrite the redirect response). Each WebSocket connection becomes a `resource` record: an NDJSON transcript of its handshake and every frame, in wire order.
8. Wraps the WARC into a **WACZ** (a ZIP, method STORE — no DEFLATE implementation needed) with a CDXJ index, `pages.jsonl`, and `datapackage.json` — [support/legalCapture/zipWriter.js](support/legalCapture/zipWriter.js). Playable in [ReplayWeb.page](https://replayweb.page) by anyone, independent of this extension.
9. Builds `manifest.json` — sha256 of every artifact actually produced (`capture.wacz`, `screenshot.png`, `page.html`, `page.mhtml` — whichever toggles left on), the TLS summary, redirect chain, network/Service-Worker coverage stats, the exact `captureOptions` used, a **tool provenance** block (extension version/id, browser `userAgent`, OS/arch via `chrome.runtime.getPlatformInfo`, local timezone offset, and **install type** via `chrome.management.getSelf()` — one of the few management APIs callable *without* the `management` permission; `"normal"` means the published Web Store build produced the package, `"development"` means an unpacked copy the operator could edit, which `report.txt` flags loudly), a **connection** block (the main document's `remoteIPAddress`/`remotePort`/`protocol`, already recorded by [networkRecorder.js](support/legalCapture/networkRecorder.js) but previously never surfaced, plus a computed `remoteAddressIsPrivateOrLoopback` flag — a loopback/RFC1918 address is the signature of a locally-run server standing in for the real site), an expanded **tls** block (cipher/key exchange/SAN list plus `certificateTransparencyCompliance` and the full SCT list — a certificate minted by a locally-installed root cannot obtain valid CT log signatures, so this distinguishes a forged cert that the browser nonetheless trusts), a **certificateChain** block (SHA-256 fingerprints of each DER cert via CDP `Network.getCertificate`, best-effort — the leaf fingerprint is searchable on crt.sh by anyone), and — each gated by its own toggle — a **machine info** block (CPU model/core count via `chrome.system.cpu`, installed RAM via `chrome.system.memory`, connected display layout/resolution via `chrome.system.display`; needs the `system.cpu`/`system.memory`/`system.display` *optional* permissions, requested only when the toggle is turned on), the operator's **Chrome account email** (`chrome.identity.getProfileUserInfo`, needs the `identity` optional permission), and **operator geolocation** (gathered in [popup.js](popup.js) via `navigator.geolocation`, not in the service worker — the Geolocation API isn't available in an MV3 service worker context — then passed through `startLegalCapture`'s `geolocation` param. No manifest permission is involved: Chrome rejects `"geolocation"` in `optional_permissions` outright — it's one of a handful of API permissions, like `debugger`/`devtools`, that can only ever be a standing permission — so this toggle instead relies on the ordinary per-origin Geolocation prompt Chrome shows the first time `navigator.geolocation.getCurrentPosition` is actually called. Turning the toggle on triggers that call immediately so a denial reverts the toggle right away instead of failing silently at capture time — but that call is deliberately *not* made from the toolbar popup itself: an undecided permission prompt steals focus, and Chrome auto-closes `action` popups on blur, which used to kill the popup (and the toggle's checked state with it) before an answer ever came back. `popup.js`'s change handler first checks `navigator.permissions.query({name:"geolocation"})` — if the state is already `granted`/`denied` there's no prompt and the popup is safe; only when the state is undecided does it message [support/legalCapture/geoPermissionRelay.js](support/legalCapture/geoPermissionRelay.js) to open [support/legalCapture/geoPermission.html](support/legalCapture/geoPermission.html) as an ordinary `chrome.windows.create` window — immune to popup-on-blur — which makes the real call and reports the outcome back for the relay to persist into `legalCaptureOptions`. `gatherGeolocation()` (the actual per-capture read, run from the popup when the Legal Capture button is clicked) guards the same way — it only calls `getCurrentPosition` when `permissions.query` already reports `granted`, skipping geolocation for that capture rather than risk a mid-capture prompt closing the popup and aborting the whole capture). All of it — including `operator.name`/`operator.caseReference` (unverified, human-typed) — is **frozen** before any timestamp request: manifest.json's own sha256 is what gets sealed, so a single hash transitively covers every other evidentiary file and metadata block inside it.
10. If timestamping is on, requests an **RFC 3161 timestamp** over `manifest.json`'s hash from whichever of **FreeTSA, DigiCert, Sectigo** are individually enabled (`TSA_PROVIDERS` in [support/legalCapture/tsaClient.js](support/legalCapture/tsaClient.js), all queried over HTTPS) concurrently, so the capture isn't sealed by one third party's availability or trustworthiness. Any succeeding is enough; more succeeding is simply stronger. The DER request is hand-encoded (no ASN.1 library); each response is independently **verified** — not just parsed — by checking its `messageImprint` hash matches what was actually sent and (when present) that its nonce echoes ours, before being accepted as a successful timestamp. Each verified response also has its embedded `genTime` diffed against this machine's local clock at request time (`clockSkewSeconds`, in `timestamps.json` and `report.txt`) — cheap cross-corroboration that preempts a "the local clock was manipulated" dispute, since independent authorities' skew readings can be compared against each other. We deliberately do not attempt to validate the TSA's CMS signature itself (that would need a full X.509 chain validator, making the report only as trustworthy as this codebase's own from-scratch one) — the raw signed token is saved untouched per authority (`capture-freetsa.tsr`, `capture-digicert.tsr`, `capture-sectigo.tsr`) for independent verification: `openssl ts -verify -digest <manifest sha256> -in capture-sectigo.tsr -CAfile <bundle>` (see **The trust boundary** below — `-reply -text` only prints a token and verifies nothing, and each authority's token is not equally checkable). Requests set `certReq` TRUE, so each token embeds its signing chain and stays verifiable without fetching anything later.
11. Downloads one zip (`legal-capture-<timestamp>.zip`, `saveAs: true`) containing whichever of `capture.wacz`, `screenshot.png`, `page.html`, `page.mhtml` were produced, `manifest.json` (frozen, hash-sealed core), `manifest.sig` + `operator-key.pem` (the installation signature), whichever `start-<authority>.tsr` pre-capture commitment tokens succeeded, `timestamps.json` (the TSA results/errors, written after — it points at manifest.json's hash rather than being inside it, since nothing can attest to its own future), whichever `capture-<authority>.tsr` files succeeded, `SHA256SUMS.txt` (sha256 of every file physically in the zip — its own toggle, a convenience check that nothing was swapped post-assembly, not itself sealed unlike manifest.json), and `report.txt` (plain-language "what this proves / doesn't prove" writeup, including a **Capture Options** section mirroring `captureOptions` exactly, the operator/case-reference/geolocation/account-email certification block, tool + machine + page-environment provenance, redirect chain, TSA verification + clock-skew results, and network/WebSocket/Service-Worker coverage stats).

**The progress banner.** Legal Capture drives [support/captureOverlay.js](support/captureOverlay.js) as a running status display, not the single "Capturing…" flash Page/Element capture use: `showCaptureOverlay(tabId, {message, placement})` now takes both, and re-injection *updates the label in place* rather than stacking a second overlay, so each phase change is one more call. Legal Capture passes `placement: "top"` — the overlay stays up for the whole session and a centred pill would sit on exactly the content being captured. Phases: starting → reloading → the post-load countdown → measuring → screenshot → packaging → timestamps → saving. It matters because the popup that launched the capture has almost always closed by then, so the banner is the only thing telling the operator a long capture is progressing rather than hung.

Two subtleties: the banner is **removed before the DOM/MHTML snapshots and re-injected after** (step 6 strips it as byte-level evidence; re-adding it once nothing else reads the DOM is safe, and it keeps the page covered through packaging, when the debugger has detached and input suppression has therefore ended). And its teardown lives in `startLegalCapture`'s **outer** `finally`, not the inner session one — it now outlives the debugger session, and no failure may strand a full-viewport `pointer-events: auto` overlay on the operator's page. `setCaptureOverlayMessage` is deliberately a no-op when the overlay isn't installed, so a phase message arriving during the snapshot window can't resurrect it into the evidence.

**Why it can silently produce nothing, and what now prevents it.** The original failure mode: an MV3 service worker is killed after ~30s with no events or `chrome.*` calls, and a pending `fetch` is not an API call. Execution simply stops — no rejection for the popup to show, nothing in `chrome://extensions`' Errors panel (which lists only *uncaught* exceptions), no download. Three things address it, and all three should stay:

- **[support/legalCapture/tsaClient.js](support/legalCapture/tsaClient.js) bounds every TSA request** (`TSA_TIMEOUT_MS`, `AbortController`). Callers run the providers concurrently under `Promise.allSettled` and therefore wait for the slowest, so one unreachable authority sets the floor for the whole phase. Measured: a network that blackholes `timestamp.digicert.com` costs Chrome ~21s before it gives up — most of the worker's budget, spent inside the one phase that makes no API calls. The abort branch is reported distinctly from a rejection ("unreachable from this network"), which is the difference between an authority worth retrying elsewhere and one that refused.
- **[support/legalCapture/workerKeepalive.js](support/legalCapture/workerKeepalive.js)** wraps the whole capture with a 20s `chrome.runtime.getPlatformInfo()` tick. It covers the tail after the debugger detaches too — WACZ assembly, zip and base64 make no `chrome.*` call either. It is a mitigation, not licence for unbounded waits; Chrome still caps a single worker task at five minutes regardless.
- **[support/legalCapture/captureTrace.js](support/legalCapture/captureTrace.js)** writes each phase into `chrome.storage.local` *as it happens*, which is what makes this class of failure diagnosable at all: storage survives both the popup closing and the worker dying. Read it with `chrome.storage.local.get("legalCaptureTrace")` in the service-worker console. A trace ending on a plain step (rather than `done`/`failed`) means the worker died there; batching the writes to the end would defeat the entire purpose.

**Three measures that constrain what a forger can do**, as distinct from the
verification work above, which only helps a reader *notice*. Each is
best-effort — none may cost the operator a capture — and each is stated in
`report.txt` with its own limits rather than as a guarantee.

- **Pre-capture commitment** ([legalCaptureSession.js](support/legalCapture/legalCaptureSession.js)'s
  `buildStartCommitment`, toggle `startTimestamp`, default on). Before the page
  is touched, `sha256("sharpshooter-legal-capture-start\n" + url + "\n" +
  nonce + "\n" + startedAt + "\n")` is timestamped by the enabled authorities
  and saved as `start-<authority>.tsr`. The seal timestamp bounds the capture
  from one end only; this bounds the other, so the operation is bracketed:
  began no earlier than T₁, sealed no later than T₂. **This is the one measure
  that bites the rebuild attack** — a forger assembling a package later cannot
  produce a start token they never requested. All three inputs are recorded in
  the sealed manifest so a verifier recomputes the hash themselves; the nonce
  is what stops commitments being precomputed for every URL in advance.
  Verified end to end against Sectigo: recomputed hash gives
  `Verification: OK`, a wrong one gives `message imprint mismatch`.
- **Capture sequence chain** ([support/legalCapture/captureChain.js](support/legalCapture/captureChain.js)).
  Each manifest records `chainId`, `sequence` and `previousManifestSha256`.
  Makes **selective disclosure visible** — nothing else in the package touches
  it, since a perfectly sealed capture says nothing about the captures that
  weren't produced. Advanced **only after the download starts**: advancing
  earlier would burn a number on a failed capture and leave a gap no package
  explains, destroying the signal. A cleared chain or a changed key restarts at
  1 under a new `chainId` with `chainRestarted: true` — disclosed, not hidden.
- **Installation signing key** ([support/legalCapture/operatorKey.js](support/legalCapture/operatorKey.js)).
  Non-extractable ECDSA P-256 in IndexedDB, signing the frozen manifest into
  `manifest.sig` with `operator-key.pem` alongside. **Continuity, not
  identity** — nobody vouched for the key; it links a series of captures to one
  installation, which is strictly more than the self-typed `operator.name` ever
  did. Signatures are converted from WebCrypto's raw r‖s to **DER**, because
  `openssl dgst -verify` cannot read P1363 and a signature no one can check is
  worth nothing. Verified: `Verified OK`, and `Verification failure` after a
  one-field edit. Do not make the key extractable to allow backups — an
  exportable key can be handed to someone else and the continuity claim dies.
- Plus **`provenance.codeIntegrity`** ([support/legalCapture/codeIntegrity.js](support/legalCapture/codeIntegrity.js)):
  SHA-256 of the modules that produce the package, so `installType` becomes
  checkable against the published build instead of purely self-asserted. The
  path list is explicit because MV3 cannot enumerate its own bundle — **add to
  it when you add a module that shapes package contents.**

**The trust boundary — which files actually carry integrity.** The chain is
`capture-<authority>.tsr` → `sha256(manifest.json)` → the files listed in
`manifest.json`'s `files` block. **Nothing else in the package carries any
integrity at all.** `report.txt`, `timestamps.json` and `SHA256SUMS.txt` are
all written *after* manifest.json is frozen (`manifest.sig`, `operator-key.pem`
and the `.tsr` tokens are outside the seal too, but unlike those three they are
**self-authenticating** — each verifies against `manifest.json` on its own, so
editing them makes them fail rather than lie) (they can't be inside a hash they
postdate), so anyone holding the zip can edit them freely. Checking a file's
hash against `report.txt` proves nothing — a swapped file plus an updated
report agree with each other perfectly. Demonstrated end to end against a real
Sectigo token: editing the report alone passes a report-based check and fails
a manifest-based one; editing the manifest to match a forged file then fails
`openssl ts -verify` with `message imprint mismatch`, which is unforgeable
without the authority's private key.

Three consequences, all load-bearing:

- `manifest.json` carries a sealed **`verification` block** stating what it
  does and doesn't cover. It is *inside* the sealed bytes deliberately — a
  statement about the trust boundary that lives outside the seal can be edited
  by the same person who edits everything else.
- `report.txt`'s **HOW TO VERIFY THIS PACKAGE** section is a five-step
  procedure with working commands, placed *before* WHAT THIS PROVES so a
  reader can check the claims before reading them. It must use
  `openssl ts -verify -digest <manifest hash>`, never `-data` (the token seals
  the manifest's *hash*, so the `-data` form checks the wrong thing and reports
  a mismatch on a sound package) and never `-reply -text`, which only prints a
  token and verifies nothing. The report said `-reply -text` for a long time;
  an examiner following it verified nothing whatsoever.
- **With no timestamp token there is no anchor.** Hashes then prove only
  internal consistency, which a forger achieves too. Both `report.txt` and the
  sealed `verification` block say so outright rather than letting the presence
  of hashes imply more than they support.

**TSA tokens are not equally checkable, and the package grades them.**
`TSA_PROVIDERS` carries `publiclyTrustedRoot`, surfaced per token in
`report.txt` and `timestamps.json`. Verified against a stock CA bundle:
Sectigo returns `Verification: OK`; **FreeTSA fails with "self-signed
certificate in certificate chain"** — its root is in no public trust store, so
verifying it means obtaining freetsa.org's own CA and electing to trust it,
from the party that issued the token. FreeTSA stays enabled (an extra
independent witness, and its `genTime` still corroborates the others' clocks),
but a contested capture should lean on a publicly-trusted token. Don't flatten
these back into an undifferentiated list.
